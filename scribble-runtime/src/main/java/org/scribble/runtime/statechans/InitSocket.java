...
public abstract class InitSocket<S extends Session, R extends Role> extends LinearSocket<S, R> implements AutoCloseable
{
	...
	public void connect(Callable<? extends BinaryChannelEndpoint> cons, Role role, String host, int port) throws ScribRuntimeException, UnknownHostException, IOException
	{
		// Can connect unlimited, as long as not already used via init
		if (isUsed())
		{
			throw new ScribRuntimeException("Socket already initialised: " + this.getClass());
		}
		try
		{
	...
	@Deprecated
	public void accept(Role role) throws IOException, ScribRuntimeException
	{
		if (isUsed())
		{
			throw new ScribRuntimeException("Socket already initialised: " + this.getClass());
	...
	@Override
	public void close() throws ScribRuntimeException
	{
		try
		{
			this.se.close();
		...
		finally
		{
			...
			{
				throw new ScribRuntimeException("Session not completed: " + this.se.self);
			}
		}
	}
}
