 ...
 * $ java -cp lib/antlr-3.1.3.jar org.antlr.Tool -o bin src/scribble/ScribbleBase.g
*/

//grammar ScribbleProtocol;
grammar ScribbleBase;  // Should be renamed in derived implementation...


/* Aim is to make a fully platform/implementation independent grammar file? Not sure if the ANTLR "import" can be used to "extend" these base rules with e.g. implementation specific rewrite rules. For now, just copy-paste base file and manually add rewrite rules etc for each implementation.
 */

options  // Implementation specific?
{
	//output=AST;  // output is needed by ANTLR, but implementation can fill this in (and there is a default)
	backtrack=true;  // Resolves some grammar issues (modulename, ...), but should it be relied on?...
}
