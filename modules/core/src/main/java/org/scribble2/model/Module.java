...
public class Module extends ModelNodeBase
{
	...
	/*public List<GlobalProtocolDecl> getGlobalProtocolDecls()
	{
		...
		return getProtocolDecls(GlobalProtocolDecl.isGlobalProtocolDecl, GlobalProtocolDecl.toGlobalProtocolDecl);
	}
	...
	public <T extends ProtocolDecl<? extends ProtocolDefinition<? extends ProtocolBlock<? extends InteractionSequence<? extends InteractionNode>>>>>
			List<T> getProtocolDecls(
					Predicate<ProtocolDecl<? extends ProtocolDefinition<? extends ProtocolBlock<? extends InteractionSequence<? extends InteractionNode>>>>> filter,
					Function<ProtocolDecl<? extends ProtocolDefinition<? extends ProtocolBlock<? extends InteractionSequence<? extends InteractionNode>>>>, T> cast)
	{
	...
	// pn is simple name
	private static <T extends ProtocolDecl<? extends ProtocolDefinition<? extends ProtocolBlock<? extends InteractionSequence<? extends InteractionNode>>>>> T
			getProtocolDecl(List<T> pds, ProtocolName pn)
	{
		List<T> filtered = pds.stream().filter((pd) -> pd.name.toName().equals(pn)).collect(Collectors.toList());
		if (filtered.size() != 1)
		{
