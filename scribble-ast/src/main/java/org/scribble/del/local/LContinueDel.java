		...
		implements LSimpleInteractionNodeDel
{
			...
			ProtocolDefInliner inl, ScribNode visited) throws ScribException
	{
		...
		LContinue inlined = inl.lang.config.af.LContinue(lc.getSource(), recvar);
			...
			ReachabilityChecker checker, ScribNode visited) throws ScribException
	{
		...
		ReachabilityEnv env = checker.popEnv()
				.addContinueLabel(lc.getRecVarChild().toName());
			...
			EGraphBuilder graph, ScribNode visited) throws ScribException
	{
		...
		else // ** "Overwrites" previous edge built by send/receive(s) leading to this continue
		{
			...
			while (preds.hasNext())
			{
				...
				graph.util.addRecursionEdge(pred, prev,
						graph.util.getRecursionEntry(rv));
